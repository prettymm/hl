...
var paths = {
    ...
    static: config.build + 'Content/',
    buildJs: config.build + 'Content/js/',
    buildLocale: config.build + 'Content/locale/',
    img: config.build + 'Content/images/form/',
    css: config.build + 'Content/css/',
    js: config.src + 'js/',
    vendorJs: config.src + 'vendor/js/',
    vendorCss: config.src + 'vendor/css/',
    coffee: config.src + 'coffee/',
    srcStylus: config.src + 'stylus/app*.styl',
    srcJs: [config.src + 'js/helpers/_*.js', config.src + 'js/modules/_*.js'],
    srcCoffee: [config.src + 'coffee/helpers/_*.coffee',
    ...
    srcLocale: config.src + 'locale/*.json',
...
gulp.task('locales', function () {
    return gulp.src(paths.srcLocale)
        .pipe(gulp.dest(paths.buildLocale));
});
...
gulp.task('server', function (cb) {
    ...
    if (!featureEnabled.noserver) {
        ...
        var nodemonCmd = (process.platform == 'win32') ? 'node_modules\\.bin\\nodemon.cmd' : './node_modules/.bin/nodemon'

        var portOps = 'PORT=' + config.port, ls = null,
            spawnOps = [portOps, nodemonCmd, './bin/www', '--ignore', 'build/', '--ignore', 'node_modules', '--ignore', 'src/', featureEnabled.lr];

        if (config.port == 80) {
            ls = spawn('sudo', spawnOps);
        } else {
            process.env.PORT = config.port;
            ls = spawn(nodemonCmd, spawnOps.slice(2));
        }
        stream(ls);
    } else {
        initServerBase();
...
gulp.task('coffee', function () {
    return gulp.src(paths.srcCoffee) /* Use an array of files instead if need to concat in order. e.g gulp.src(['_file1.coffee', '_file2.coffee']) */
        ...
        .pipe(toggle(insert.prepend, featureEnabled.deploy, {
            params: config.header + '\n(function(){"use strict";',
            name: 'deploy - wrap prepend'
        }))
...
gulp.task('watch-vendor', function () {
    ...
    watch(paths.vendorCss, function (file) {
        changedFile = file;
        unsetBusy();
